...
CP.snp <- baf.model.fit(Bf = seg.i$Bf[filter.size],
                        ...
                        dna.index = seq(0.5,3,0.05),
                        priors.labels = 2, priors.values = 2)
@
...
cp.plot(CP.snp)
cp.plot.contours(CP.snp, add = TRUE)
@ 
      \caption{Result from the Bayesian inference over the defined range of cellularity and DNA-index from artificial SNP array data. The color indicate the log-likelihood of the corresponding cellularity/DNA-index combinations.}
      \label{fig:cpPlotSNP}
\end{figure}

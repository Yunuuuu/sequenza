...
<<label=depthRGCnormall, echo=TRUE, eval=FALSE>>=
gc.stats <- gc.sample.stats(data.file)
@

<<label=depthRGCnorm, echo=TRUE, results=hide>>=
...
gc.vect  <- setNames(gc.stats$raw[,3], gc.stats$gc.values)
...
abf.hom  <- abf.data$ref.zigosity == 'hom'
...
weights.seg  <- 200 + round((seg.filtered$end.pos - seg.filtered$start.pos)/1e6, 0)
seg.filtered$Bf[seg.filtered$Bf>=0.4] <- 0.5
@
...
CP <- baf.model.fit(Bf = seg.filtered$Bf, depth.ratio = seg.filtered$depth.ratio, 
                    ...
                    by.p = 0.1, by.c = 0.05, avg.depth.ratio = 1,
...
snp.r.win <- windowValues(x = 2^logR$S2,
             positions = logR$pos,
             chromosomes = logR$chrs,
             window = 1e6, overlap = 5e5)
@
\subsection{Windowing B-allele frequencies values.}
<<label=BAFWin, eval=TRUE, results=hide>>=
BAF.to05 <- BAF$S2
BAF.to05[BAF.to05 >= 0.5] <- 1 - BAF.to05[BAF.to05 >= 0.5]
snp.b.win <- windowValues(x = BAF$S1,
             positions = BAF$pos,
             chromosomes = BAF$chrs,
             window = 1e6, overlap = 5e5)
@
...
allele.seg <- allele.seg[allele.seg$sampleID == "S2", ]
@

\subsection{Using the Bayesian inference on segmented SNP arrays}
<<label=adjustSegs, eval=TRUE>>=
segs.BAF.to05 <- allele.seg$BAF.mean
segs.BAF.to05[segs.BAF.to05 >= 0.5] <- 1 - segs.BAF.to05[segs.BAF.to05 >= 0.5]

avg.unlogR <- 2^(mean(allele.seg$logR.mean))
@

<<label=CPforSNP, eval = TRUE, results=hide>>=
CP.snp <- baf.model.fit(Bf = segs.BAF.to05[allele.seg$chrom == 1], depth.ratio = 2^(allele.seg$logR.mean[allele.seg$chrom == 1]), 
                    weight.ratio = allele.seg$n.probes[allele.seg$chrom == 1],
                    weight.Bf = allele.seg$n.probes[allele.seg$chrom == 1],
                    by.p = 0.1, by.c = 0.05, avg.depth.ratio = avg.unlogR,
                    cellularity.range = c(0.1,1), 
                    dna.content.range = c(0.5,3), mc.cores=4)
@
...
snp.seg.cn <- baf.bayes(Bf = segs.BAF.to05, depth.ratio = 2^(allele.seg$logR.mean), 
                        weight.ratio = allele.seg$n.probes,
                        weight.Bf = allele.seg$n.probes,
                        avg.depth.ratio = avg.unlogR,
                        cellularity = cellularity,
                        dna.content = dna.content, CNt.max = 10)

segments.snp <- cbind(allele.seg[,c(2, 4, 5, 8, 7, 6)], snp.seg.cn)
colnames(segments.snp) <- c("chromosome", "start.pos", "end.pos", "Bf","depth.ratio","N.BAF", "CNt", "A", "B", "L")
segments.snp$Bf <- segs.BAF.to05
segments.snp$depth.ratio <- 2^(allele.seg$logR.mean)
@
...
\begin{figure}[H]
      \centering
<<label=chrViewNoMutCP, fig=TRUE, width=6, height=6>>=

chromosome.view(baf.windows = snp.b.win$windows[[1]], 
                ratio.windows = snp.r.win$windows[[1]],  min.N.ratio = 1,
                segments = segments.snp[segments.snp$chromosome == "1", ],
                cellularity = cellularity, dna.content = dna.content,
                avg.depth.ratio = avg.unlogR)
@ 
      \caption{Plots B-allele frequencies (top) and un-logged-logR (bottom) with SNP array data. Horizontal dotted line indicate different copy number/ allelic state. }
      \label{fig:crhomViewNoMutCP}
\end{figure}


%\newpage
%Random citation \cite{rsvgtipsdevice}

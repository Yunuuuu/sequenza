...
plotWindows(abf.b.win[[1]], ylim = c(0, 0.5), 
           main = names(abf.r.win)[1], xlab = "Base pairs",
...
\section{Infer for cellularity and DNA-index}
...
cp.plot(CP)
@ 
      \caption{Result from the Bayesian inference over the defined range of cellularity and DNA-index. The color indicate the log-likelihood of the corresponding cellularity/DNA-index values.}
      \label{fig:cpPlot}
\end{figure}
...
par(mfrow = c(2,2))
   ...
   plot(dna.c.cint$values[,c(1,2)], xlab = "DNA-index",
...
%dna.index <- max.param[1]
%cellularity <- max.param[2]
%@


\subsection{Cellularity and DNA-index plot for SNP array}
\begin{figure}[H]
      \centering
<<label=CPplotSNP,fig=TRUE,width=6,height=6>>=
cp.plot(CP.snp)
@ 
      \caption{Result from the Bayesian inference over the defined range of cellularity and DNA-index from artificial SNP array data. The color indicate the log-likelihood of the corresponding cellularity/DNA-index values.}
      \label{fig:cpPlotSNP}
\end{figure}
...
snp.seg.cn <- baf.bayes(Bf = seg.i$Bf,
                        ...
                        dna.index = dna.index, CNt.max = 10)

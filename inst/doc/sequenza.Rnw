...
\begin{lstlisting}[style=BashInputStyle]
   sequenza-utils.py seqz-binning -w  50 \
                                  -a out.seqz.gz | gzip > out_small.seqz.gz
\end{lstlisting}
...
The object resulting from \texttt{sequenza.fit} has two vectors, \textit{ploidy} and \textit{cellularity}, indicating respectively the tested values of ploidy and cellularity, and a matrix, \textit{loglik}, with columns and rows corresponding to the previously indicated vectors, containing the estimated log-likelihood for the combinations of cellularity and ploidy.

...
Aletrnatively, it is possible to use the output of VarScan2\cite{Koboldt2012} (http://varscan.sourceforge.net), which would require a simlar approach and the generation of pileups as well.

\subsection{Obtainig the required ABfreq file.}
\subsubsection{Processing the data with \textit{sequenza-utils.py}.}
For convenience and efficiency we have implemented pre-processing algorithms in a standalone (not called from R) Python program. Although, the program is provided with the package; its exact location can be found like this:
...
\subsubsection{Convereting VarScan2 output to ABfreq.}
Since many projects might already being processed trough VarScan2, it can be conveninet to be able to import such results.
...
For whole genome sequencing the information in the \textit{varscan.snp} could be enought to estimate the ploidy and cellularity, and define the copy number and mutations, hence the \texit{varscan.copynumber} argument is optional, but it is strongly suggested to use it in case of exome sequencing.
...
Each chromosome can be visualize using the function \textit{chromosome.view} as in figure \ref{fig:3panelsPlot}. The same function can be used to visualize the data after the estimation of \textit{cellularity} and \textit{ploidy} parameters as in Figure \ref{fig:crhomViewWithCP}.
...
\subsection{Inference of cellularity and ploidy (Ploidy)}
...
The object resulting from \textit{sequenza.fit} have two vectors, x and y, indicating respectively the tested values of ploidy and cellularity, and a matrix z with x columns and y rows, containing the estimated log-likelihood.
...
By exploring the result for  \textit{cellularity} and \textit{ploidy} separately, it is possible to draw the likelihood distribution for each parameter. The information is returned by the \textit{get.ci} function.
\begin{figure}[H]
      \centering

...
Each chromosome can be visualize using the function \textit{chromosome.view} as in figure \ref{fig:3panelsPlot}. The same function can be used to visualize the data after the estimation of \textit{cellularity} and \textit{dna.index} parameters as in Figure \ref{fig:crhomViewWithCP}.

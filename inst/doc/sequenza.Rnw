...
weights.seg  <- 100 + round((seg.filtered$end.pos - seg.filtered$start.pos) / 1e6, 0)
...
weights.snp   <- 100 + round((segmented.snp$end.pos - segmented.snp$start.pos)/1e6 , 0)
...
snp.seg.cn <- baf.bayes(Bf = segmented.snp$Bf,
                        depth.ratio = segmented.snp$depth.ratio, 

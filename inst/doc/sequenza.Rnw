...
weights.snp   <- 150 + round((segmented.snp$end.pos - segmented.snp$start.pos)/1e6 , 0)
filter.size   <- (segmented.snp$end.pos - segmented.snp$start.pos) >= 3e6
@

<<label=CPforSNP, eval = TRUE, results=hide>>=
CP.snp <- baf.model.fit(Bf = segmented.snp$Bf[filter.size],
                        depth.ratio = segmented.snp$depth.ratio[filter.size], 

...
weights.seg  <- round((seg.filtered$end.pos - seg.filtered$start.pos) / 1e6, 0)
...
snp.b.win <- windowValues(x = BAF.to05,
             positions = BAF$pos,
...
fake.abf <- data.frame(chromosome = BAF$chrs, n.base = BAF$pos,
                 Bf = BAF.to05, adjusted.ratio = 2^logR$S2,
                 good.s.reads = 1, depth.sample = 1,
                 ref.zigosity = 'het')

segmented.snp <- segment.breaks(abf.tab = fake.abf, breaks = allele.seg[, c(2, 4, 5)])

avg.unlogR <- mean(segmented.snp$depth.ratio)
@

<<label=CPforSNP, eval = TRUE, results=hide>>=
CP.snp <- baf.model.fit(Bf = segmented.snp$Bf, depth.ratio = segmented.snp$depth.ratio, 
                    weight.ratio = segmented.snp$N.BAF,
                    weight.Bf = segmented.snp$N.ratio,
...
snp.seg.cn <- baf.bayes(Bf = segmented.snp$Bf, depth.ratio = segmented.snp$depth.ratio, 
                        weight.ratio = segmented.snp$N.ratio,
                        weight.Bf = segmented.snp$N.BAF,
                        avg.depth.ratio = avg.unlogR,
                        cellularity = cellularity,
                        dna.content = dna.content, CNt.max = 10)

segmented.snp <- cbind(segmented.snp, snp.seg.cn)
@

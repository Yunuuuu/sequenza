...
baf.bayes <- function(Bf, depth.ratio, cellularity, ploidy, avg.depth.ratio,
   ...
   bayes.fit <- function (x, mat, model.pts, priors, ratio.priority) {
      ...
      score.r    <- dt2(sd = mat[x,]$sd.ratio/log(mat[x,]$N.ratio), mean = mat[x,]$ratio, x = test.ratio, df = length(unique(test.ratio)), log = TRUE)
      score.r    <- score.r + log(priors)
      if (!is.na(mat[x,]$Bf)) {
         #score.b    <- baf.dpois(baf = mat[x,]$Bf, depth.t = mat[x,]$N.Bf, test.baf, log = TRUE)
         score.b    <- dt2(mean = mat[x,]$Bf, sd = mat[x,]$sd.Bf/log(mat[x,]$N.Bf), x = test.baf, df = length(unique(test.baf)), log = TRUE)

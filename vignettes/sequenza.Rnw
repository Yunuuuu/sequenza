...
\subsection{Obtaining the required ABfreq file.}
...
\begin{lstlisting}[style=BashInputStyle]
   sequenza-utils.py abfreq-binning -w  50 \
                                   out.abfreq.gz | gzip > out_small.abfreq.gz
\end{lstlisting}
...
\subsubsection{Converting VarScan2 output to ABfreq.}
...
For this purpose a simple function is provided within the package, to convert the output of the \textit{somatic} and \textit{copynumber} programs of the VarScan2 suite into the \textit{ABfreq} format.
...
The function \textit{sequenza.extract} is designed to efficiently access the raw ABfreq data and take care of normalization steps. The arguments enable customization of a set of actions listed below:
\begin{itemize}
...
chromosome.view(mut.tab = test$mutations[[3]], baf.windows = test$BAF[[3]], 
                ...
                cellularity = cellularity, ploidy = ploidy,
                avg.depth.ratio = avg.depth.ratio, avg.depth = test$avg.depth)
@ 

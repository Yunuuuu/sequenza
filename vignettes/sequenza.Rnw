...
Alternatively, it is possible to use the output of VarScan2\cite{Koboldt2012} (http://varscan.sourceforge.net), which would require a similar approach and the generation of pileups as well.

\subsection{Obtaining the required seqz file.}
\subsubsection{Processing the data with \textit{sequenza-utils.py}.}
...
To obtain the GC content information (required to obtain an \textit{seqz} file), it is possible to use a function from \textit{sequenza-utils}, and extract the average GC content using a fixed genomic windows, or download the gc5Base from golden path (\url{http://hgdownload-test.cse.ucsc.edu/goldenPath/hg19/gc5Base/}).
...
When the GC content file is available, it is possible to process the two pileup files to obtain an \textit{seqz} file containing genotype information, alleles and mutation frequency, and more other features.
\begin{lstlisting}[style=BashInputStyle]
   sequenza-utils.py pileup2seqz -gc hg19.gc50Base.txt.gz \
                                   -r normal.pileup.gz \
                                   -s tumor.pileup.gz | gzip > out.seqz.gz
\end{lstlisting}

If you don't yet have the pileup, or you are not interested in storing the pileup for further use, you can use two \textit{FIFO} files, to pipe the samtools output directly to \textit{sequenza-utils}:
\begin{lstlisting}[style=BashInputStyle]
   ...
   sequenza-utils.py pileup2seqz -gc hg19.gc50Base.txt.gz \
                                   -r normal.fifo \
                                   -s tumor.fifo | gzip > out.seqz.gz
   rm normal.fifo tumor.fifo
\end{lstlisting}

To compress further the results, it is possible to use a binning function provided in \textit{sequenza-utils}.
This would decrease the memory requirement to load all the available positions in memory. as well it would speedup the processing of the sample:

\begin{lstlisting}[style=BashInputStyle]
   sequenza-utils.py seqz-binning -w  50 \
                                   out.seqz.gz | gzip > out_small.seqz.gz
\end{lstlisting}

Where the parameter \textit{-w} indicate a window size in nucleotides, to be used for the binning.
The heterozygous and the position carrying variant calls would remain untouched. 

\subsubsection{Converting VarScan2 output to seqz.}
...
For whole genome sequencing the information in the \textit{varscan.snp} could be enough to estimate the ploidy and cellularity, and define the copy number and mutations, hence the \textit{varscan.copynumber} argument is optional, but it is strongly suggested to use it in case of exome sequencing.

\section{Exploring the \textit{seqz} file and GC-correction details}

After the aligned sequence data have been pre-processed, the \textit{sequenza} R package handles all the normalization and analysis steps. So the following part of this vignette will take place in R.
...
data.file <-  system.file("data", "data.seqz.txt.gz", package = "sequenza")
data.file
@
...
The files can be read even faster; after mapping the chromosomes location in the file, it is possible to select the coordinate (in terms of from line x to line y) of the file to read. See the man page of \textit{read.seqz} for an example.

\subsection{Quality control and normalization}

Each aligned base, in the next generation sequencing, is associated with a quality score. The \textit{sequenza-utils} software is capable of filtering the base with a quality score lower then a specified value (default, 20). The number of reads that have passed the filter is returned in the column \textit{good.s.reads}, while the \textit{depth.sample} column contains the raw depth indicated in the pileup.
...
The R package \textit{sequenza} offers an ensemble of functions and models that can be used to design customized workflows and analyses. Here we provide generic and most commonly used analysis steps. 

\begin{itemize}
\item Extract the relevant information from the raw \textit{seqz} file.
\item Fit the \textit{sequenza} model to infer cellularity and ploidy (ploidy).
...
The function \textit{sequenza.extract} is designed to efficiently access the raw seqz data and take care of normalization steps. The arguments enable customization of a set of actions listed below:
\begin{itemize}
...
names(test)
@
After the raw data is processed, the size of the data is considerably reduced. For instance the R object resulting from \textit{sequenza.extract} can be stored as a file of a few megabytes, even for whole genome sequencing data. 
...
Each chromosome can be visualized using the function \textit{chromosome.view} as in Figure \ref{fig:3panelsPlot}. The same function can be used to visualize the data after the estimation of \textit{cellularity} and \textit{ploidy} parameters as in Figure \ref{fig:crhomViewWithCP}.
...
@ 
      \caption{Plots of mutant allele frequency (top), B-allele frequency (middle) and depth ratio (bottom) for chromosome position.}
...
The function \textit{sequenza.fit} performs the inference using the calculated B allele frequency and depth ratio of the obtained segments. The method can be explored in more detail by reading the manual pages for the function \textit{baf.model.fit}.
...
The result is a list in the format \textit{list(x, y, z)}, which is directly usable by standard graphical functions, such as \textit{image}. However we provide functions to explore and better display the results, and to extract the point estimate and confidence intervals.
...
The object resulting from \textit{sequenza.fit} has two vectors, x and y, indicating respectively the tested values of ploidy and cellularity, and a matrix z with x columns and y rows, containing the estimated log-likelihood.
...
@ 
 \caption{Result from the Bayesian inference over the defined range of cellularity and ploidy. Color intensity indicates the log-likelihood of corresponding cellularity/ploidy values.}
      \label{fig:cpPlot}
\end{figure}

By exploring the results for \textit{cellularity} and \textit{ploidy} separately, it is possible to draw the likelihood distribution for each parameter. The information is returned by the \textit{get.ci} function.
\begin{figure}[H]
      \centering
...
@ 
      \caption{Plot of the log-likelihood with respective cellularity and ploidy probability distribution and confidence intervals.}
      \label{fig:cpPlotCI}
\end{figure}
...
To detect copy number variations we use a B allele frequency model, implemented in the function \textit{baf.bayes}, with the estimated parameters of \textit{cellularity} and \textit{ploidy}:
...
head(seg.tab)
@
The result consists of four values for every imputed segment: \textit{CNt} is the estimated copy number of the tumor of the given segment; \textit{A} is the estimated number of A alleles; \textit{B} is the estimated number of B alleles; \textit{L} is the log-likelihood of the model fit.

\subsection{Visualize detected copy number changes and variant alleles}

To visualize the data after detection of CNV and variant alleles, it is possible to use the \textit{chromosome.view}. In order to draw the relative model points (and to evaluate how the estimated model fits the real data) more information is needed compared to Figure \ref{fig:3panelsPlot}:
\begin{itemize}
...
@
      \caption{Genome-wide copy number profile obtained from exome sequencing.}
...
@
      \caption{Genome-wide A and B alleles profile obtained from exome sequencing.}
